...
class FDNConfig(BaseModel):
    ...
    def delay_length_samps(self) -> List[int]:
        ...
        delay_range_samps = ms_to_samps(
            np.asarray(self.delay_range_ms), self.sample_rate
        )
    ...
    def check_delays_length(cls, v, values):
        if v is not None:
            if len(v) != values.data["N"]:
                raise ValueError(
                    f"Length of delays ({len(v)}) must match N ({values['N']})"
                )
        return v
    ...
    def validate_training_device(cls, value):
        ...
        if value == "cuda":
            assert torch.cuda.is_available(), "CUDA is not available for training"
    ...
    def check_loss_alpha_length(cls, values: Dict):
        ...
        if losses is not None and alphas is not None:
            if len(losses) != len(alphas):
                raise ValueError(
                    "The number of losses must be equal to the number of alphas"
                )
        return values
