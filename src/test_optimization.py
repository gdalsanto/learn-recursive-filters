...
def main(config_dict):
    ...
    config["criteria"] = unpack_functions(losses, config_dict.losses, config_dict.device)
    ...
    fc_values = 2 * np.pi * np.random.uniform(6000, 12000, size=config_dict.num_targets) / config['fdn'].sample_rate
    # --- Trainer config_dict ---
    for i_target in range(config_dict.num_targets):
        ...
        scipy.io.savemat(os.path.join(config['output_dir'], "target_values.mat"), {"rt": cur_rt, "fc": cur_fc})
        ...
        target_fdn.model.get_core().feedback_loop.feedforward.attenuation.assign_value(
            torch.tensor([cur_rt, cur_fc], dtype=torch.float64, device=config_dict.device)
        )
        ...
        if config["add_noise_to_target"]:
            ...
            for i_crit in range(len(config["criteria"])):
                if config["criteria"][i_crit].add_noise: 
                    config['criteria'][i_crit].noise_file = os.path.join(config["output_dir"],"noise_term.mat")
        ...
        dataset = Dataset(
            ...
            expand=config["trainer_config"].num              # this coincides with the number of iterations gradient descent per epoch
        )
        ...
        sf.write(rir_file, target_fdn_ir.squeeze().detach().cpu().numpy(), config["fdn"].sample_rate)
        ...
        for i_crit, criterion in enumerate(config["criteria"]):
            ...
            init_fc = 2 * np.pi * np.random.uniform(6000, 12000, 1) / config['fdn'].sample_rate
            fdn.model.get_core().feedback_loop.feedforward.attenuation.assign_value(
                torch.tensor([init_rt, init_fc], dtype=torch.float64, device=config_dict.device).squeeze()
            )
            ...
            trainer = Trainer(
                ...
                step_size=config["trainer_config"].step_size,          # Learning rate scheduler step size
                patience_delta=config["trainer_config"].patience_delta,   # Early stopping threshold
            ...
            trainer.register_criterion(
                ...
                alpha=config["trainer_config"].sparsity_weight, # Weight for sparsity loss
                requires_model=True # This loss needs access to model parameters
            )
            # Register the main criterion
            trainer.register_criterion(        
                criterion, 
                alpha=config["loss_weights"][i_crit], 
            ...
            scipy.io.savemat(param_file, {"optim_parameters": optimized_params, "energy_target_fdn": energy_target_fdn.detach().cpu().numpy(), "energy_optimized_fdn": energy_fdn.detach().cpu().numpy()})
            scipy.io.savemat(param_file.replace("parameters", "loss"), {"loss": trainer.train_loss})
            ...
            print(f"Final validation loss for criterion {criterion.name}: {trainer.valid_loss[-1]:.6f}")
            core = fdn.model.get_core()
            attenuation_parameters = core.feedback_loop.feedforward.attenuation.param
            print(f"Optimized attenuation parameters: {attenuation_parameters.detach().cpu().numpy()}")
